...
<div id="wrapper">
	...
	while($row=mysqli_fetch_assoc($result)){
		echo "<form action=mcommentresult.php method='post'>";
		echo "<input type=hidden value=".$row["NO"]." name='NO'>";
		echo "<input type=hidden value=".$row["id"]." name='id'>";
		echo "<h3>留言：<input type=text value=".$row["comment"]." name='comment' style='font-family:Microsoft JhengHei;'></br></h3>";
		echo "<input type=hidden value=".$row["time"]." name='time'></br>";

		echo "<input type='submit'style='font-family:Microsoft JhengHei;'>";
		echo "<a href=mcomment1.php>取消</a>";
		echo "</form>";
	...
	$SQLUpdate="select * from comment WHERE NO1='$NO1'";

	if($result=mysqli_query($link,$SQLUpdate)){
	while($row=mysqli_fetch_assoc($result)){
		echo "<form action=mcommentresult.php method='post'>";
		echo "<input type=hidden value=".$row["NO1"]." name='NO1'>";
		echo "<input type=hidden value=".$row["id"]." name='id'>";
		echo "<h3>留言:<input type=text value=".$row["comment"]." name='comment' style='width:600px;font-family:Microsoft JhengHei;'></br></h3>";
		echo "<input type=hidden value=".$row["time"]." name='time'></br>";

		echo "<input type='submit'style='font-family:Microsoft JhengHei;'>";
		echo "<a href=mcomment.php>取消</a>";
		echo "</form>";
