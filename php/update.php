...
<div id="wrapper">
	...
	if($result=mysqli_query($link,$SQLUpdate)){
		while($row=mysqli_fetch_assoc($result)){
			...
			echo "<h3>使用者名稱:<input type=text value=".$row["username"]." name='name' style='font-family:Microsoft JhengHei;'></br></h3>";
			echo "<input type=hidden value=".$row["id"]." name='id'>";
			echo "<h3>密碼:<input type=password value=".$row["password"]." name='pwd'></br></h3></br>";

			echo "<h2><input type='submit' value='確定' style='height:28px;font-size:16px;font-family:Microsoft JhengHei;'> <a href=user.php>取消</a></h2>";
			echo "</form>";
		}
	}
